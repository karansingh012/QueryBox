      ...
      <div className="flex-1 flex flex-col">
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 m-4 rounded-lg">
            ...
            <p className="text-sm mt-2">Make sure the backend server is running on http://127.0.0.1:5001</p>
            <button 
  ...
  const handleExport = async (format) => {
    if (format === 'JSON') {
      ...
      try {
        ...
        doc.text('🎯 Interview Summary Report', 20, 30);
        
        // Date and basic info
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Generated on: ${new Date().toLocaleString()}`, 20, 45);
        doc.text(`Session ID: ${summaryData.sessionId || 'N/A'}`, 20, 55);
        doc.text(`Role: ${summaryData.role || 'N/A'}`, 20, 65);
        
        // Overall Score
        doc.setFontSize(18);
        doc.setTextColor(40, 40, 40);
        doc.text('📊 Overall Performance', 20, 85);
        doc.setFontSize(14);
        doc.setTextColor(39, 174, 96); // Green
        doc.text(`Score: ${summaryData.overallScore}/10`, 25, 100);
        doc.setTextColor(100, 100, 100);
        doc.text(`Questions Completed: ${summaryData.totalQuestions}`, 25, 110);
        ...
        doc.setTextColor(100, 100, 100);
        let yPosition = 145;
        summaryData.strengths.forEach((strength, index) => {
        ...
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        summaryData.weaknesses.forEach((weakness, index) => {
        ...
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        summaryData.resources.forEach((resource, index) => {
          const lines = doc.splitTextToSize(`${index + 1}. ${resource}`, 170);
          doc.text(lines, 25, yPosition);
        ...
        for (let i = 1; i <= pageCount; i++) {
          ...
          doc.text('Generated by QueryBox AI Interview Platform', 20, 285);
          doc.text(`Page ${i} of ${pageCount}`, 170, 285);
        }
        
        // Save the PDF
        doc.save(`interview-summary-${Date.now()}.pdf`);
        
      } catch (error) {
        console.error('PDF generation failed:', error);
        alert('Failed to generate PDF. Please try again or export as JSON.');
      }
    }
  };
